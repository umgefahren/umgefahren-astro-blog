...
In the summer of 2020, near the end of 11th grade, I worked with a friend for a start-up where we built an MVP for them. The product we delivered worked, but it was built horribly, built by teenagers
not knowing what they were doing (maybe I'll describe the tech stack once, but it is incredibly awful). After we finished the project, we reviewed how we could improve. I decided to learn a new 
programming language, faster and better than Python (I went for Rust) and he decided to improve his general programming skills by building a big project in C.
I showed him [termchat](https://github.com/lemunozm/termchat), which we then went on using in school and we were fascinated by the idea. He continued writing a similar project, which he never
finished, and I dug into the weeds on how termchat worked. In hindsight it is actually quite simple. It's actually just utilizing mDNS to find other users and then connecting to them. 
I liked the idea but wanted to build something people could use over the internet (basically IRC).

At this point it's important to point out how little I knew and still don't know about the topics I was dealing with back then and to this day. I'm a self-taught programmer and I mainly did robotics and
...
I soon figured I wanted to build something *like* [IRC](https://en.wikipedia.org/wiki/Internet_Relay_Chat) but without central servers needed to communicate but also very anonymous. 
...
retrieve and delete Key-Value pairs in `O(log n)`, because it's essentially a binary tree.
...
when messages were lost. However, as previously stated, it's not that simple to write a P2P network from scratch.
...
```
Note the trait implementing a conversion between a kademlia event and the composed event. If any of these parts are not perfect, it will just not work. The documentation is not helpful and the compiler
...
```
Notice the command receiver. At a different point in your code you have to define this channel and also a Command Enum that encapsulates everything you want to do. The sender part of this channel is
also your only way to use the protocols. You basically send a command, the command is received and the swarm is instructed to perform the lookup. You are given a QueryId you have to save, along with
...
This actually quite lengthy article concludes the journey of a self-taught programmer high school student building a system too complex for the 2000s over the summer. I'm currently improving the
